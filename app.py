...
app = Flask(__name__)
app.config['MONGO_URI'] = 'mongodb+srv://juanpedrobastiastorresi:nfqxYH6uOUUPRRDi@cluster0.yreqjlm.mongodb.net/mydatabase?retryWrites=true&w=majority'
mongo = PyMongo(app)
...
def scripts():
    if request.method == 'GET':
        ...
        return jsonify([{'id': str(script['_id']), 'text': script['text']} for script in scripts])
    elif request.method == 'POST':
        data = request.get_json()
        new_script = {'text': data['script']}
        mongo.db.scripts.insert_one(new_script)
        created_script = mongo.db.scripts.find_one(new_script)
        return jsonify({'id': str(created_script['_id']), 'text': created_script['text']}), 201
    else:
...
def script(script_id):
    if request.method == 'DELETE':
        ...
        result = mongo.db.scripts.update_one({'_id': ObjectId(script_id)}, {'$set': updated_script})
